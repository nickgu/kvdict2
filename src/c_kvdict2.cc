 ...
 *      ¿ÉÒÔÃÖ²¹Python×Ô´ø´ÊµäÐÔÄÜÎÊÌâ¡£
 *       ÄÚ´æKV´Êµä£º
 *       - ½«ÎÄ¼þ¶ÁÈëKey-ValueÐÅÏ¢£¨Ä¿Ç°¾ÖÏÞ¶¼ÊÇÎÄ±¾£©
 *       - ½«¹þÏ£ÐÅÏ¢ÐòÁÐ»¯µ½ÎÄ¼þ£¨·½±ãÔÚ´Ëµ÷ÓÃ£©
 *       Ó²ÅÌKVË÷Òý´Êµä
 *       - ¹¹½¨´óÎÄ¼þµÄË÷Òý
 *  
 **/
#include <python2.7/Python.h> //°üº¬pythonµÄÍ·ÎÄ¼þ
        ...
        bool load_in_memory) 
{
    ...
    while (fgets(line, MAX_LINE_LENGTH, fp)) {
        ...
        // Ã¿Ò»ÐÐ¶¼»á±»Â¼Èë£¬Èç¹ûÖ»ÓÐÒ»ÁÐ£¬ÔòÕûÁÐ×÷Îªkey£¬valueÎª¿Õ
...
static PyMethodDef CKVDictFunc[] = {
    ...
    // ½«Ë÷ÒýºÍ¶ÔÓ¦µÄÎÄ¼þ¶ÁÈëµ½Ó²ÅÌ´Êµä
...
PyMODINIT_FUNC initc_kvdict2(void) {
    //³õÊ¼Ä£¿é£¬°ÑCKVDictFunc³õÊ¼µ½c_kvdictÖÐ
